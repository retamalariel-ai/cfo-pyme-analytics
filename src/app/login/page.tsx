...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    console.log('Login exitoso. Usuario:', data.user?.email, '| Sesión:', data.session?.access_token?.slice(0, 20) + '…');
    router.refresh(); // flush server-side session cache first
    router.push('/'); // then navigate — middleware will see the cookie
  };
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex w-full flex-col justify-between bg-[#050a12] px-10 py-10 md:w-2/5">
        ...
        <div className="mx-auto w-full max-w-sm">
          <h1 className="mb-1.5 text-2xl font-bold text-slate-50">Iniciar sesión</h1>
      ...
      <div className="relative hidden overflow-hidden bg-[#070e1a] md:flex md:w-3/5 items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col gap-4 text-left">
          {[
            { label: 'Punto de Equilibrio', value: '$22.500', sub: 'en ventas', color: 'text-emerald-400' },
            { label: 'Margen de Seguridad', value: '34.2%',   sub: 'sobre proyección', color: 'text-blue-400' },
            { label: 'EBITDA Estimado',     value: '$8.320',  sub: 'con ventas actuales', color: 'text-amber-400' },
